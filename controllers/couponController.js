...
const loadCoupon=async (req,res)=>{
    ...
    const loadEditCoupon=async (req,res)=>{
      try{
        ...
        if(Coupon){
          // var newDate =new Date(Coupon[0].expiryDate)
          // var date1=newDate.utc().format("YYYY-MM-DD")
  ...
  const editCoupon = async (req, res) => {
    ...
    try {
        ...
        let expirationDate=req.body.expiryDate
          ...
          const  couponData=   await coupon.findOneAndUpdate({_id:req.params.id},{$set:{code:code, description:description
               ,offerPrice:offerPrice,minimumAmount:minimumAmount,expiryDate: expirationDate}}); 
            if(couponData){
                  // res.render('couponList',{coupons:coupons,isAdded:true}); 
